...
$sql = "SELECT * FROM slaves WHERE `owner` = '" . \libAllure\Session::getUser()->getUsername() . "'";
$result = $db->query($sql);
...
while ($row = $result->fetchRow()) {
    popup("<li>" . $row['name'] . "</li>", "view_slave.php?slave=" . $row['id']);
}
echo "</ul>";
stopBox(BOX_GREEN);

$sql = 'SELECT * FROM slaves WHERE owner = ""';
$result = $db->query($sql);
...
if ($result->numRows() == 0) {
    ...
    echo "There are <strong>" . $result->numRows() . "</strong> slaves for sale. <a href = shop.php?mode=slaves>buy some</a>.";
}
stopBox(BOX_NULL);
